...
+
Specifies the compression encoding to use when downloading LFS objects
over HTTP. Valid values are `gzip` or `zstd`. Default is `gzip`.
+
This option may be applied selectively to some URLs, following the same
matching rules as those defined for Git's `http.<url>.*`
link:https://git-scm.com/docs/git-config#Documentation/git-config.txt-httplturlgt[options].
For example, to set this option only for a specific host, use:
+
....
...
+
Specifies the maximum time in seconds LFS will wait before aborting a
transfer when a server requests a delay using the `Retry-After` header.
If the server indicates a wait time longer than this value, LFS will
log a warning and mark the transfer as failed rather than sleeping for
the full requested duration.
+
Must be an integer which is at least one. If the value is not an integer,
is less than one, or is not given, a value of 300 will be used instead.
