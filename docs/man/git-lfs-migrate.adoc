...
----
*git lfs migrate* _<mode>_ [_options_] [*--*] [_branch_ ...]
----
...
`--yes`::
  ...
  are not careful.
`--rewrite-messages`::
  Replace commit hashes in commit messages with their rewritten counterparts.
`[branch ...]`::
...
=== Migrate without rewriting local history (using migrate import --no-rewrite)

You can also migrate files without modifying the existing history of
your repository.

Migrating to Git LFS in a single new commit, without specifying a commit message:
...
Note that the `*.mp3` and `*.psd` filename patterns are not quoted and
so will be expanded by the shell, not by the `git lfs migrate import`
command.  See the remarks following the next example for further details.

Migrating to Git LFS in a single new commit, with a specified commit message:
...
$ git lfs migrate import --no-rewrite --yes \
  -m "Import test.zip, audios/*.mp3, and images/*.psd files" \
  test.zip audios/*.mp3 images/*.psd
....

Note that the `*.mp3` and `*.psd` filename patterns are not quoted and
so will be expanded by the shell, not by the `git lfs migrate import`
command. Using quotation marks around such patterns with the
`--no-rewrite` option is not supported, as the command expects a list
of files (possibly as expanded by the shell) and will not search for
all files matching a quoted filename pattern.

Additional commands may be used to find and list all files matching a set
of patterns, of course. In a POSIX-compliant environment such as Linux,
macOS, or Git Bash on Windows, the example above could be combined with
the `find(1)` and `sed(1)` commands as follows, so long as the resulting
list of files does not cause the length of the expanded command line to
exceed the maximum permitted by the shell:

....
$ git lfs migrate import --no-rewrite --yes \
  $(find . -name "*.zip" -o -name "*.mp3" -o -name "*.psd" | sed "s/^\.\///")
....

Note also that the `git lfs track` command will by default change the
modification timestamps on any existing files which match the given
filename patterns so the files will appear "dirty" to Git and thereby
prompt the user to convert them to Git LFS files. The subsequent
`git lfs migrate import` command will then caution that the working tree
contains modifications and ask the user for confirmation to proceed
unless the `--yes` option is specified, as in the examples above.

=== Migrate without rewriting local history (using add --renormalize)

As an alternative to using `git lfs migrate import` with the `--no-rewrite`
option to migrate files to Git LFS without modifying the existing history
of your repository, if you have Git v2.16.0 or higher installed, you can
use the `--renormalize` option of the `git add` command to achieve
...
$ git add .gitattributes
$ git add --renormalize .
$ git commit -m "Import all *.zip, *.mp3, and *.psd files to Git LFS"
....
