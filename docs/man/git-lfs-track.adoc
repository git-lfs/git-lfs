...
`--min-size`::
   ...
   option is provided, Git LFS is configured to automatically convert any file
   exceeding the threshold to LFS. The `lfs.autotracksize` option is set in
   `.lfsconfig` and `* filter=lfs` is added to `.gitattributes`. Existing files
   larger than the threshold are touched so they will be re-indexed by Git on
   the next `git add`.
   +
   Both `.lfsconfig` and `.gitattributes` must be committed to the repository
   for the autotrack setting to apply to all clones.
...
* Automatically track all files larger than 10 megabytes, persisting the
   setting in `.lfsconfig`:
+
