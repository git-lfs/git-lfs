...
----
*git lfs track* [_options_] [_<pattern>_...]
----
...
`--min-size`::
   ...
   Uses both decimal (KB, MB, GB) and binary (KiB, MiB, GiB) units. When this
   option is provided, the threshold is written as a custom attribute in
   `.gitattributes` as `* filter=lfs autotracksize=<bytes>`. Existing files
   larger than the threshold are touched so they will be re-indexed by Git on
   the next `git add`.
   +
