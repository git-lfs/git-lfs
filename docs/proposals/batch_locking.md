...
The client sends the following to create lock sby sending a `POST` to `/locks/batch` (appended to the LFS server url).
...
Server should reject locks creation if one already exists for **any** of the paths requested.
...
The client can delete locks, given their IDs, by sending a `DELETE` request to `/locks/batch` (appended to the LFS server url).
...
Server should reject locks deletion if *any* of the locks could not be deleted.
