...
gerenated in ```./repos/{DISTRO_NAME}/{VERSION #}```. Just drop the repo
...
```./docker``` directory, since that is the root they are built against 
...
* Run another command (like bash) in a running docker
    ...
    docker exec -i {dockcer name} {command
