...
func NewDownloadCheckQueue(files int, size int64) *TransferQueue {
	// Always dry run
	return newTransferQueue(files, size, true, transfer.GetDownloadAdapter(transfer.BasicAdapterName))
}
...
func NewDownloadQueue(files int, size int64, dryRun bool) *TransferQueue {
	return newTransferQueue(files, size, dryRun, transfer.GetDownloadAdapter(transfer.BasicAdapterName))
}
