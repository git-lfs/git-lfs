...
func TestPrePushWithEmptyQueue(t *testing.T) {
	...
	cmd := repo.Command("pre-push", "origin", "master")
	cmd.Output = ""
}
...
func TestPrePushToMaster(t *testing.T) {
	...
	cmd := repo.Command("pre-push", "--dry-run", "origin", "master")
...
func TestPrePushToNewBranch(t *testing.T) {
	repo := NewRepository(t, "empty")
	defer repo.Test()

	cmd := repo.Command("pre-push", "--dry-run", "origin", "newbranch")
	cmd.Output = "push a.dat\npush b.dat"
